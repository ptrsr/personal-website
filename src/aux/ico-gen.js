...
// retrieved from https://github.com/mourner/icomesh
...
export default function icomesh(order = 4, uvMap = false) {
    ...
    const test = new Float32Array(numVertices + numDuplicates);
    ...
    for (let i = 0; i < order; i++) { // repeatedly subdivide each triangle into 4 triangles
        ...
        for (let k = 0; k < prevLen; k += 3) {
            ...
            const a = addMidPoint(v1, v2, i);
            const b = addMidPoint(v2, v3, i);
            const c = addMidPoint(v3, v1, i);
            let t = k * 4;
            test[v1] = i;
            test[v2] = i;
            test[v3] = i;
    ...
    if (!uvMap) return {vertices, triangles, test};
